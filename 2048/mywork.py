    ...
    for episodes in range(num_episodes):
        ...
        while not done:
            ...
            if len(replay_buffer)>=batch_size:
                ...
                target_q_values = rewards+gamma*(1-done)*next_q_values
        ...
        if episodes %update_target_freq==0:
            target_model.load_state_dict(model.state_dict())
        
        epsilon = max(epsilon*epsilon_decay,epsilon_min)
        print(f"Episode{episodes}:Total Reward = {total_reward} , Epsilon = {epsilon}")
    torch.save(model.state_dict(), "dqn_2048.pth")
    print("模型已保存为 dqn_2048.pth")
    plt.close()
